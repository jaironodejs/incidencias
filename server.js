 ...
 */
app.get("/form_incidencia", (req, res) => {
  res.render("pages/form", {
    rutaActual: "/form_incidencia",
  });
});
 ...
 * Ruta para Reportes
 */
app.get("/rep_incidencia", (req, res) => {
  connection.query('SELECT * FROM estudiantes',(error, results)=>{
    if(error){
        throw error;
    } else {                       
      res.render("pages/reporte", {results:results,
        rutaActual: "/rep_incidencia"
      });
    }   
});
